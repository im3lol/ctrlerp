...
function getLinkedDocumentLabel(ret: PurchaseReturn): string {
  if (ret.purchaseReceipt) return ret.purchaseReceipt.number
  if (ret.purchaseInvoice) return ret.purchaseInvoice.number
  if (ret.purchaseOrder) return ret.purchaseOrder.number
  return '—'
}

function getLinkedDocumentTypeLabel(ret: PurchaseReturn): string {
  if (ret.purchaseReceipt) return 'إذن استلام'
  if (ret.purchaseInvoice) return 'فاتورة شراء'
  if (ret.purchaseOrder) return 'أمر شراء'
  return ''
}
...
export default function PurchaseReturnsList() {
  ...
  const fetchReturns = async () => {
    try {
      ...
      const res = await fetch(`/api/purchases/returns?companyId=${companyId}&${params.toString()}`)
      if (res.ok) {
        const data = await res.json()
        setReturns(data)
      }
    } catch {
      toast.error('فشل في تحميل مرتجعات المشتريات')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleViewDetail = async (returnId: string) => {
    ...
    try {
      const res = await fetch(`/api/purchases/returns/${returnId}?companyId=${companyId}`)
      if (res.ok) {
        setDetailReturn(await res.json())
      }
    } catch {
      toast.error('فشل في تحميل تفاصيل مرتجع المشتريات')
    } finally {
  ...
  const handleAction = async () => {
    ...
    setSubmitting(true)
    try {
      const res = await fetch(`/api/purchases/returns/${confirmReturnId}?companyId=${companyId}`, {
        method: 'PUT',
      ...
      if (res.ok) {
        toast.success(
          confirmAction === 'confirm' ? 'تم تأكيد مرتجع المشتريات بنجاح' : 'تم إلغاء مرتجع المشتريات'
        )
        fetchReturns()
      } else {
    ...
    <>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  {returns.length.toLocaleString('ar-EG')} مرتجع مشتريات
                </p>
              </div>
            </div>
            <Button
                ...
                {returns.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={8} className="text-center py-12">
                      <div className="flex flex-col items-center text-slate-400">
                        ...
                        <p className="text-sm">لا توجد مرتجعات مشتريات</p>
                        <p className="text-xs mt-1 text-slate-300">
                          اضغط على &quot;إضافة مرتجع&quot; لإنشاء مرتجع مشتريات جديد
                        </p>
                      </div>
                    </TableCell>
                  </TableRow>
                  ...
                  returns.map((ret) => (
                    ...
                    >
                      ...
                      <TableCell className="font-medium">{ret.supplier.nameAr}</TableCell>
      ...
      <Dialog open={detailDialogOpen} onOpenChange={setDetailDialogOpen}>
        <DialogContent className="sm:max-w-3xl max-h-[85vh]">
          <DialogHeader>
            <DialogTitle>تفاصيل مرتجع المشتريات</DialogTitle>
          </DialogHeader>
          {detailLoading ? (
            ...
            <div className="space-y-4 py-2 max-h-[65vh] overflow-y-auto">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium">{detailReturn.supplier.nameAr}</p>
                </div>
                <div>
      ...
      <Dialog open={confirmDialogOpen} onOpenChange={setConfirmDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {confirmAction === 'confirm' ? 'تأكيد مرتجع المشتريات' : 'إلغاء مرتجع المشتريات'}
            </DialogTitle>
            <DialogDescription>
              {confirmAction === 'confirm'
                ? 'سيتم تأكيد مرتجع المشتريات وإنشاء حركات مخزنية صادرة. هل أنت متأكد؟'
                : 'سيتم إلغاء مرتجع المشتريات. هل أنت متأكد؟'}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
